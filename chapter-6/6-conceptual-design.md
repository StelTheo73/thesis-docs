...
After determining the genre, mechanics and GBL elements that would be used in the game, selecting the educational content and designing the evaluation process, we started designing the development process. We decided that our first step would be to convert the conceptual design into a wireframe. The next step would be to define the screens of the application and the navigation flow between them (application structure). After evaluating the wireframe and the application structure, we would start developing the game. The implementation of the first two steps would allow us to create a working prototype, which would then be tested by typical users. Based on their feedback, we will improve the prototype and repeat this process until we have a satisfactory result. Then the alpha version of the game will be ready, and we will be able to conduct extensive user testing.

### Level Wireframe

![Wireframe](chapter-6/image003_wireframe.png){#fig:fig602 height=50%}

A prototype level (wireframe) was then designed in _diagrams.net_^[https://app.diagrams.net/], which formed the foundation for creating the game\'s graphical interface. As shown in _Figure 6.2_, the original approach did not include a graphical element for the quantum register, but rather 2 discrete qubits to which the quantum gate would be applied. There were also two buttons, one for applying the gate and one for launching the missiles. The target state was placed at the top of the screen. Finally, the targets were enemy spaceships, as in the original _Space Invaders_ game.
